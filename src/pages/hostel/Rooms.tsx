...
import {
  ...
  Wrench, Users, X, Search, Filter, Hash,
} from "lucide-react"
...
const STATUS_CFG: Record<RoomStatus, { label: string; color: string; bg: string; dot: string }> = {
  available:   { label: "Available",   color: T.green,  bg: "rgba(16,185,129,.12)", dot: T.green  },
  occupied:    { label: "Occupied",    color: T.accent, bg: "rgba(99,102,241,.12)", dot: T.accent },
  maintenance: { label: "Maintenance", color: T.amber,  bg: "rgba(245,158,11,.12)", dot: T.amber  },
}
...
function OccupancyRing({ room }: { room: Room }) {
  ...
  const color = pct === 0 ? T.textMut : pct < 60 ? T.green : pct < 85 ? T.amber : T.red
  return (
    <div style={{ position:"relative", width:56, height:56, flexShrink:0 }}>
      ...
      <div style={{
        ...
        color, fontSize:11, fontWeight:700, lineHeight:1.1, fontFamily:"'JetBrains Mono',monospace",
      }}>
        {pct}%
      </div>
    </div>
...
function StatCard({ icon, label, value, color, isLoading }: {
  ...
  return (
    <div style={{
      ...
      onMouseEnter={(e)=>{ (e.currentTarget as HTMLDivElement).style.transform="translateY(-2px)"; (e.currentTarget as HTMLDivElement).style.boxShadow=`0 8px 32px rgba(0,0,0,.4)` }}
    ...
    >
      ...
      <div>
        <div style={{ fontSize:12, color:T.textSec, fontWeight:500, marginBottom:4 }}>{label}</div>
        <div style={{
          fontSize:28, fontWeight:800, color:T.textPri, lineHeight:1,
          fontFamily:"'DM Serif Display',serif",
        }}>
          {isLoading ? <span style={{ display:"inline-block", width:48, height:24, borderRadius:6, background:"rgba(255,255,255,0.06)", animation:"shimmer 1.4s ease-in-out infinite" }} /> : value}
        </div>
      </div>
    </div>
  )
...
function RoomCard({ room, onView, onDelete }: {
  room: Room; onView: () => void; onDelete: () => void
}) {
  const [hovered, setHovered] = useState(false)
  return (
    <div
      style={{
        background: hovered ? "#1e2338" : T.card,
    ...
    >
      ...
      <div style={{
        ...
        background: STATUS_CFG[room.status]?.dot ?? T.accent,
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:16 }}>
        <div>
          <div style={{
            ...
            fontSize:20, fontWeight:700, color:T.textPri, letterSpacing:"-0.02em",
          }}>
            {room.room_no}
          </div>
          <div style={{ fontSize:11, color:T.textMut, marginTop:2 }}>{room.type}</div>
        </div>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div>
          <div style={{ fontSize:11, color:T.textMut, marginBottom:4 }}>Block</div>
        ...
        <div style={{ textAlign:"right" }}>
          <div style={{ fontSize:11, color:T.textMut, marginBottom:4 }}>Monthly</div>
      ...
      >
        <button
          ...
          style={{
            ...
            background:"rgba(99,102,241,.12)", color:T.accent,
        ...
        <button
          onClick={onDelete}
          style={{
            padding:"8px 12px", borderRadius:10, border:`1px solid rgba(239,68,68,.2)`,
            background:"rgba(239,68,68,.08)", color:T.red,
...
function RoomForm({ initialValues, onSubmit, isLoading, error, submitLabel, onCancel, token }: {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    background:"rgba(255,255,255,0.04)", color:T.textPri, fontSize:13,
    outline:"none", transition:"border-color .2s",
  }
  const labelStyle: React.CSSProperties = { fontSize:11, fontWeight:600, color:T.textSec, marginBottom:6, display:"block", textTransform:"uppercase", letterSpacing:"0.06em" }
    ...
    <form onSubmit={async e => { e.preventDefault(); await onSubmit(form) }}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:16, marginBottom:16 }}>
        {[
          ...
          <div key={f.name}>
            <label style={labelStyle}>{f.label}{f.required && <span style={{color:T.red}}>*</span>}</label>
            <input type={f.type} name={f.name} value={f.value} onChange={set} required={f.required}
        ...
        <div>
          <label style={labelStyle}>Block<span style={{color:T.red}}>*</span></label>
      ...
      {error && (
        <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 14px", borderRadius:10, background:"rgba(239,68,68,.12)", color:T.red, fontSize:12, marginBottom:16 }}>
      ...
      <div style={{ display:"flex", gap:10 }}>
        <button type="submit" disabled={isLoading} style={{
          ...
          background: isLoading ? "rgba(99,102,241,.4)" : T.accent,
        ...
        <div style={{
          ...
          <div style={{ display:"flex", alignItems:"center", gap:16 }}>
            ...
            <div>
              <div style={{
                fontFamily:"'DM Serif Display',serif",
                fontSize:22, fontWeight:700, color:T.textPri,
              }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            {!isLoading && room && mode==="view" && (
              <button onClick={()=>setMode("edit")} style={{
                ...
                background:"transparent", color:T.textSec, fontSize:12, fontWeight:600,
        ...
        <div style={{ overflowY:"auto", padding:"24px 28px", flex:1 }}>
          {isLoading ? (
            <div style={{ display:"flex", justifyContent:"center", padding:"40px 0" }}>
              <Loader2 size={28} color={T.accent} style={{animation:"spin .8s linear infinite"}} />
            </div>
          ) : error || !room ? (
            <div style={{ padding:"14px", borderRadius:12, background:"rgba(239,68,68,.12)", color:T.red, fontSize:13 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:24 }}>
              {/* Left: details + status */}
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:T.textMut, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:16 }}>Room Details</div>

                {/* Big occupancy ring */}
                <div style={{
                  display:"flex", alignItems:"center", gap:20, padding:20,
                  background:T.card, borderRadius:16, border:`1px solid ${T.border}`, marginBottom:16,
                }}>
                  <OccupancyRing room={room} />
                  <div>
                    <div style={{ fontSize:13, color:T.textSec, marginBottom:4 }}>
                      <span style={{ color:T.textPri, fontWeight:700, fontFamily:"'JetBrains Mono',monospace" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                  {[
                    ...
                    <div key={f.label} style={{
                      ...
                      <div style={{ fontSize:10, color:T.textMut, textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:4 }}>{f.label}</div>
                      <div style={{ fontSize:14, fontWeight:700, color: (f as any).accent ? T.amber : T.textPri }}>
                        {f.value}
                      </div>
                    </div>
                  ))}
                ...
                <div style={{ marginTop:20 }}>
                  <div style={{ fontSize:11, fontWeight:700, color:T.textMut, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:10 }}>Change Status</div>
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                    {(["available","occupied","maintenance"] as RoomStatus[]).map(s => {
                      const c = STATUS_CFG[s]
                      const active = room.status === s
                      return (
                        <button key={s} disabled={active || updateStatus.isPending}
                          onClick={() => updateStatus.mutate({ id:room._id, status:s })}
                          style={{
                            padding:"7px 16px", borderRadius:20,
                            border:`1px solid ${active ? c.dot : T.border}`,
                  ...
                  {updateStatus.error && (
                    <div style={{ marginTop:10, fontSize:12, color:T.red, display:"flex", alignItems:"center", gap:6 }}>
              ...
              {/* Right: occupants */}
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:T.textMut, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:16 }}>
                    ...
                    {room.occupants.map((occ, i) => (
                      <div key={occ._id} style={{
                        ...
                        <div style={{ minWidth:0 }}>
                          <div style={{ fontSize:13, fontWeight:600, color:T.textPri, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                            {occ.student_name}
                          </div>
                          <div style={{ fontSize:11, color:T.textMut, fontFamily:"'JetBrains Mono',monospace" }}>
                            #{occ.student_roll_no}
                          </div>
                        </div>
                        <div style={{ marginLeft:"auto", flexShrink:0 }}>
                          <span style={{
                            ...
                            background:"rgba(16,185,129,.12)", padding:"3px 8px", borderRadius:8,
                          }}>Active</span>
                        </div>
                      </div>
                    ))}
...
function DeleteModal({ room, onConfirm, onCancel, isLoading, error }: {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background:T.surface, border:`1px solid rgba(239,68,68,.25)`,
        borderRadius:20, padding:28, width:"100%", maxWidth:420,
        boxShadow:"0 40px 80px rgba(0,0,0,.6)",
      }}>
        <div style={{ width:52, height:52, borderRadius:16, background:"rgba(239,68,68,.12)", color:T.red, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:20 }}>
          <Trash2 size={24} />
        </div>
        <div style={{ fontSize:18, fontWeight:700, color:T.textPri, marginBottom:8, fontFamily:"'DM Serif Display',serif" }}>Delete Room</div>
        <div style={{ fontSize:13, color:T.textSec, marginBottom:20, lineHeight:1.6 }}>
          This will permanently remove <strong style={{color:T.textPri}}>Room {room.room_no}</strong>. The room must have no occupants before deletion.
        </div>
        {error && (
          <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 14px", borderRadius:10, background:"rgba(239,68,68,.12)", color:T.red, fontSize:12, marginBottom:16 }}>
            <AlertTriangle size={13} />{error}
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={onConfirm} disabled={isLoading} style={{
            ...
            background: isLoading ? "rgba(239,68,68,.4)" : T.red,
            color:"#fff", fontSize:13, fontWeight:600, cursor:"pointer",
...
const Rooms: React.FC = () => {
  ...
  return (
    <div style={{ background:T.bg, minHeight:"100vh", padding:"28px 32px", fontFamily:"'DM Sans',sans-serif", color:T.textPri }}>
      ...
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        select option { background:#1e2338; color:#f1f5f9; }
      `}</style>

      {/* Header */}
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:32, animation:"fadeUp .4s ease" }}>
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:6 }}>
            <div style={{ width:10, height:10, borderRadius:"50%", background:T.accent, boxShadow:`0 0 12px ${T.accent}` }} />
            <span style={{ fontSize:11, fontWeight:700, color:T.accent, textTransform:"uppercase", letterSpacing:"0.1em" }}>Hostel Management</span>
          </div>
          <h1 style={{ margin:0, fontSize:32, fontWeight:800, fontFamily:"'DM Serif Display',serif", color:T.textPri, lineHeight:1.1 }}>Rooms</h1>
          <p style={{ margin:"6px 0 0", fontSize:13, color:T.textSec }}>Monitor occupancy, assign rooms, and manage availability</p>
        </div>
        <button
          onClick={() => setShowCreate(p => !p)}
          style={{
            padding:"11px 22px", borderRadius:12, border:"none",
            background: showCreate ? "rgba(99,102,241,.3)" : T.accent,
            color:"#fff", fontSize:13, fontWeight:700, cursor:"pointer",
            display:"flex", alignItems:"center", gap:8,
            transition:"all .2s", boxShadow: showCreate ? "none" : `0 0 24px rgba(99,102,241,.35)`,
          }}
        >
      ...
      {/* Stats */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:16, marginBottom:28, animation:"fadeUp .4s ease .05s both" }}>
        <StatCard icon={<BedDouble size={22} />}     label="Total Rooms"     value={stats?.totalRooms     ?? 0}         color={T.accent}  isLoading={roomStats.isLoading} />
        <StatCard icon={<CheckCircle2 size={22} />}  label="Available Beds"  value={stats?.availableBeds  ?? 0}         color={T.green}   isLoading={roomStats.isLoading} />
        <StatCard icon={<Users size={22} />}          label="Occupancy Rate"  value={`${stats?.occupancyRate ?? 0}%`}   color={T.amber}   isLoading={roomStats.isLoading} />
        <StatCard icon={<Wrench size={22} />}         label="Maintenance"     value={stats?.byStatus?.maintenance ?? 0} color={T.red}     isLoading={roomStats.isLoading} />
      </div>

      {/* Create form */}
      {showCreate && (
        <div style={{
          background:T.card, border:`1px solid rgba(99,102,241,.25)`, borderRadius:20,
          padding:28, marginBottom:24, animation:"fadeUp .3s ease",
          boxShadow:`0 0 40px rgba(99,102,241,.08)`,
        }}>
          <div style={{ fontSize:15, fontWeight:700, color:T.textPri, marginBottom:20, display:"flex", alignItems:"center", gap:10 }}>
            <Plus size={16} color={T.accent} /> Add New Room
          </div>
      ...
      {/* Filters */}
      <div style={{ display:"flex", gap:12, marginBottom:24, flexWrap:"wrap", animation:"fadeUp .4s ease .1s both" }}>
        <div style={{ position:"relative", flex:1, minWidth:220 }}>
          <Search size={14} color={T.textMut} style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)" }} />
          <input
        ...
        {[
          ...
          <select key={f.name} value={f.val}
            ...
            style={{
              ...
              background:"rgba(255,255,255,.03)", color: f.val ? T.textPri : T.textMut,
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, padding:"0 16px", borderRadius:12, border:`1px solid ${T.border}`, background:"rgba(255,255,255,.02)", fontSize:12, color:T.textMut, whiteSpace:"nowrap" }}>
      ...
      {/* Error */}
      {error && (
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"14px 18px", borderRadius:12, background:"rgba(239,68,68,.1)", color:T.red, fontSize:13, marginBottom:20 }}>
      ...
      {isLoading ? (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:16 }}>
          {Array.from({length:6}).map((_,i) => (
            <div key={i} style={{ background:T.card, borderRadius:16, padding:20, border:`1px solid ${T.border}`, height:180, animation:"shimmer 1.4s ease-in-out infinite", opacity:.5 }} />
          ))}
        </div>
      ) : rooms.length === 0 ? (
        <div style={{ textAlign:"center", padding:"80px 0", color:T.textMut }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:16, animation:"fadeUp .4s ease .15s both" }}>
          {rooms.map((room, i) => (
            <div key={room._id} style={{ animation:`fadeUp .3s ease ${i*.04}s both` }}>
              <RoomCard room={room} onView={()=>setSelectedId(room._id)} onDelete={()=>setDeleteTarget(room)} />
            </div>
          ))}
        </div>
      )}
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:28, paddingTop:24, borderTop:`1px solid ${T.border}` }}>
          <span style={{ fontSize:12, color:T.textMut }}>Page {filters.page ?? 1} of {totalPages}</span>
          <div style={{ display:"flex", gap:8 }}>
            {[{ label:"← Prev", disabled:(filters.page??1)<=1, action:()=>setFilters({page:(filters.page??1)-1}) },
              { label:"Next →", disabled:(filters.page??1)>=totalPages, action:()=>setFilters({page:(filters.page??1)+1}) }].map(b => (
              <button key={b.label} onClick={b.action} disabled={b.disabled || isLoading} style={{
                ...
                background: b.disabled ? "transparent" : "rgba(255,255,255,.04)",
                color: b.disabled ? T.textMut : T.textSec,
      ...
      {deleteTarget && (
        <DeleteModal room={deleteTarget} onConfirm={handleDelete} onCancel={()=>setDeleteTarget(null)}
          isLoading={deleteRoom.isPending} error={deleteRoom.error?.message ?? null} />
      )}
    </div>
  )
}
