...
const FeeTemplateAdmin: React.FC = () => {
  ...
  const [viewTemplate, setViewTemplate] = useState<FeeTemplate | null>(null);
  ...
  const validateForm = (): boolean => {
    ...
    formData.lineItems.forEach((item:any, idx) => {
      if (!item.description.trim()) newErrors[`lineItem${idx}Desc`] = 'Required';
      if (!item.amount || Number(item.amount) <= 0) newErrors[`lineItem${idx}Amount`] = 'Invalid';
    });
  ...
  const handleSave = () => {
    ...
    if (editingTemplate) {
      setTemplates(templates.map(t => t.id === editingTemplate.id ? { ...formData, id: t.id, total } as FeeTemplate : t));
    } else {
      setTemplates([...templates, { ...formData, id: Date.now(), total } as FeeTemplate]);
    }
    resetForm();
    setView('list');
  };
              ...
              <div className="row g-3">
                ...
                <div className="col-md-6">
                  ...
                  <input
                    ...
                    onChange={(e: ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, roomType: e.target.value })}
                  />
                ...
                <div className="col-md-6">
                  ...
                  <select
                    ...
                    onChange={(e: ChangeEvent<HTMLSelectElement>) => setFormData({ ...formData, frequency: e.target.value })}
                  >
                  ...
                  <div key={idx} className="row g-2 mb-2 align-items-start">
                    <div className="col">
                      <input
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => updateLineItem(idx, 'description', e.target.value)}
                      />
                    </div>
                    <div className="col-3">
                      <input
                        ...
                        placeholder="Amount"
                        value={item.amount}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => updateLineItem(idx, 'amount', e.target.value)}
                      />
                    </div>
        ...
        <div className="card shadow-sm border-0">
          <div className="card-header bg-white py-3 border-bottom">
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
              ...
              <button onClick={() => setView('form')} className="btn btn-primary d-flex align-items-center gap-2">
          ...
          <div className="card-body p-4">
            <div className="row g-3 mb-4">
              ...
              <div className="col-6 col-md-3 col-lg-2">
                <select className="form-select" value={categoryFilter} onChange={(e: ChangeEvent<HTMLSelectElement>) => setCategoryFilter(e.target.value)}>
              ...
              <div className="col-6 col-md-3 col-lg-2">
                <select className="form-select" value={frequencyFilter} onChange={(e: ChangeEvent<HTMLSelectElement>) => setFrequencyFilter(e.target.value)}>
                ...
                <tbody>
                  {filteredTemplates.map((template) => (
                    <tr key={template.id}>
                      ...
                      <td>
                        <span className={`badge ${template.category === 'Room' ? 'bg-primary' : 'bg-success'}`}>
                          {template.category}
                        </span>
                      </td>
                      <td>{template.frequency}</td>
                      <td>{template.roomType}</td>
                      <td className="text-end fw-bold">₹{template.total.toLocaleString()}</td>
                      <td>
                        <div className="d-flex justify-content-center gap-1">
                          <button onClick={() => setViewTemplate(template)} className="btn btn-sm btn-outline-info">
                            <i className="fas fa-eye"></i>
                          </button>
                          <button onClick={() => handleEdit(template)} className="btn btn-sm btn-outline-warning"> 
                            <i className="fas fa-edit"></i>
                          </button>
                          <button onClick={() => setDeleteConfirm(template)} className="btn btn-sm btn-outline-danger">
            ...
            <div className="card shadow" style={{ width: '100%', maxWidth: '600px' }}>
              ...
              <div className="card-body p-4">
                <div className="row g-4">
                  <div className="col-12">
                    <label className="text-muted small d-block mb-1 fw-bold">Description</label>
                    <p>{viewTemplate.description || 'No description provided'}</p>
                  </div>
                  <div className="col-4">
                    <label className="text-muted small d-block mb-1 fw-bold">Category</label>
                    <p className="mb-0">{viewTemplate.category}</p>
                  </div>
                  <div className="col-4">
                    <label className="text-muted small d-block mb-1 fw-bold">Frequency</label>
                    <p className="mb-0">{viewTemplate.frequency}</p>
                  </div>
                  <div className="col-4">
                    <label className="text-muted small d-block mb-1 fw-bold">Room Type</label>
                    <p className="mb-0">{viewTemplate.roomType}</p>
                  </div>
                  <div className="col-12">
                    <label className="text-muted small d-block mb-2 fw-bold">Line Items</label>
                    <div className="list-group">
                      ...
                      <div className="list-group-item d-flex justify-content-between bg-primary text-white fw-bold">
                        <span>Total</span>
