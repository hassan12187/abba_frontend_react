import React, { useState, type KeyboardEvent,type ChangeEvent,type FormEvent } from 'react';
...
export default function ForgotPasswordPage() {
  ...
  const handleEmailSubmit = async (e: FormEvent | KeyboardEvent) => {
    e.preventDefault();
    setError('');
  ...
  const handleCodeSubmit = async (e: FormEvent | KeyboardEvent) => {
    e.preventDefault();
    setError('');
  ...
  const handlePasswordSubmit = async (e: FormEvent | KeyboardEvent) => {
    e.preventDefault();
    setError('');
    ...
    setTimeout(() => {
      setLoading(false);
      if (responseStatus === 401 || responseStatus === 500) return;
      setStep(4);
    }, 1500);
  };
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        {step === 4 && (
          <div style={{ ...styles.formContent, textAlign: 'center', padding: '2rem 0' }}>
            ...
            <a href="/login" style={styles.submitButton as React.CSSProperties}>
              Go to Login
            </a>
          </div>
        )}
...
// Global styles (remains largely the same, but handled in a side-effect if needed)
