...
interface CreateFeeInvoiceProps {
  onBack?: () => void;
  onSave?: (data: any) => void;
}

const CreateFeeInvoice: React.FC<CreateFeeInvoiceProps> = ({ onBack, onSave }) => {
  const navigate = useNavigate();
  const { token } = useCustom();

  // --- Constants & Templates ---
  const feeTemplates: FeeTemplate[] = [
  ...
  // --- Calculations ---
  const totalAmount: number = formData?.lineItems.reduce(
  ...
  const handleSearch = async (): Promise<void> => {
    ...
    try {
      ...
      if (response) {
        setFoundStudent(response as Student); 
      } else {
  ...
  const confirmStudentSelection = (): void => {
  ...
  // --- Handlers: Fee Items ---
  const applyTemplate = (e: ChangeEvent<HTMLSelectElement>): void => {
  ...
  const handleLineItemChange = (index: number, field: keyof LineItem, value: string): void => {
                ...
                <div className="row g-3">
                  ...
                  <div className="col-md-6">
                    <InputField 
                      ...
                      onChange={(e: ChangeEvent<HTMLInputElement>) => setFormData({...formData, billingMonth: e.target.value})} 
                    />
                  </div>
                  <div className="col-md-6">
                    <InputField 
