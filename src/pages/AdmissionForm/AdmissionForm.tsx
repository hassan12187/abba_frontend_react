...
const AdmissionForm: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    const requiredFields: (keyof AdmissionFormData)[] = [
      'student_roll_no', 'academic_year', 'student_name', 'father_name', 'cnic_no', 
      'date_of_birth', 'gender', 'student_email', 'student_cellphone', 
      'active_whatsapp_no', 'guardian_name', 'guardian_cellphone', 
      'postal_address', 'permanent_address', 'city', 'province'
    ];
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await Axios.post('/static/admission-form', dataToSend, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });

      if (result.status === 200) {
        enqueueSnackbar(result.data.message || "Application Submitted Successfully", { 
          variant: "success",
